...
def compute_sparse_vector_similarity(features1, features2):
    ...
    size = max(int(features1['size']), int(features2['size']))
    ...
    vec2 = csr_matrix((np.array(features2['values'], dtype=float),
                       (np.zeros(len(features2['indices'])), np.array(features2['indices'], dtype=int))), shape=(1, size))
