...
def get_topic_distribution(preprocessed_data):
        data = news_processor.feauture_engineering(preprocessed_data)
        topics_distribution = news_processor.lda_model.transform(data)
        max_index_udf = udf(lambda topicDistribution: int(np.argmax(topicDistribution)), IntegerType())
        return topics_distribution.withColumn("most_dominant_topic", max_index_udf(topics_distribution.topicDistribution))
...
def get_topic_distribution(preprocessed_data):
    ...
    max_index_udf = udf(lambda topicDistribution: int(np.argmax(topicDistribution)), IntegerType())
    return topics_distribution.withColumn("most_dominant_topic", max_index_udf(topics_distribution.topicDistribution))
#get_topic_distribution(news_with_sentiment_df)

# Get topic distribution
# Get topic distribution within the streaming context
#query = news_with_sentiment_df.writeStream \

query = get_topic_distribution(news_with_sentiment_df) \
    .outputMode("append") \
    .format("console") \
    .start()
